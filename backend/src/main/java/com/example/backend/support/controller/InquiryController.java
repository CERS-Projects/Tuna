...
public class InquiryController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<InquiryEntity> getInquiryById(@PathVariable("id") @NonNull String id) {
        try {
            InquiryEntity inquiry = inquiryService.findByInquiryId(id);
            if (inquiry != null) {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteInquiry(@PathVariable("id") @NonNull String id) {
        try {
            inquiryService.deleteInquiry( id );
            System.out.println("Deleted inquiry with id: " + id);
            return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
        } catch (Exception e) {
            // エラーハンドリング（例：ログ出力、適切なレスポンスコードの返却など）
            e.printStackTrace();
