...
const Material = () => {
	const { id } = useParams();
	const { data: Datialdata, isFetching, isError } = useClassroom(id);
	...
	const allDocuments =
		Datialdata?.categories?.flatMap((cat) => cat.documents) ?? [];

	if (isFetching) return <p>読み込み中...</p>;
	if (isError || !Datialdata) return <p>教室の取得に失敗しました。</p>;
		...
		<div className={styles.materialLayout}>
			<div className={styles.materialContainer}>
				<div className={styles.materialMain}>
					<div className={styles.feedWrapper}>
						...
						<div className={styles.headerArea}>
							<h1>{Datialdata.roomName}</h1>
							<p>{Datialdata.description}</p>
						</div>
				...
				<div className={styles.materialSub}>
					<InfoBox>
						<MaterialCategory categories={Datialdata.categories} />
					</InfoBox>
				</div>
			</div>
