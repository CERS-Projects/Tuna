...
const Material = () => {
	const { id } = useParams();
	const { data: classroomDetail, isFetching, isError } = useClassroom(id);
	...
	if (isError || !classroomDetail) return <p>教室の取得に失敗しました。</p>;
		...
		<div className={styles.materialLayout}>
			<div className={styles.materialContainer}>
				<div className={styles.materialMain}>
					<div className={styles.feedWrapper}>
						...
						<div className={styles.headerArea}>
							<h1>{classroomDetail.roomName}</h1>
							<p>{classroomDetail.description}</p>
						</div>
				...
				<div className={styles.materialSub}>
					<InfoBox>
						<MaterialCategory categories={classroomDetail.categories} />
					</InfoBox>
				</div>
			</div>
