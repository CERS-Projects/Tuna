...
export const ClassroomCategoryUploader = <T extends FieldValues>({
	...
	const isEditMode = "categoryId" in value;
		...
		if (e.target.files) {
			...
			const duplicate = files.find(
				(file) =>
					...
					existingDocs.some((doc) => doc.documentName === file.name),
			);
			if (duplicate) {
	...
	const handleRemoveExisting = (doc: ClassroomCategoryDocument) => {
		if (isEditMode && onUpdate) {
			...
			onUpdate({
				...v,
				existingDocuments: v.existingDocuments.filter(
			...
			<div className={styles.header}>
				<div className={styles.headerContent}>
					<input
						...
						type="text"
						placeholder="カテゴリ名"
						{...register(`categories.${index}.categoryName` as Path<T>, {
							required: "必須項目です",
						})}
					/>
				...
				<div className={styles.fileListArea}>
					...
					{existingDocs.map((doc, i) => (
						<div className={styles.fileIconBox} key={`exist-${i}`}>
							...
							<span className={styles.fileName}>{doc.documentName} </span>
							<button
								type="button"
